...
import { getAllSalesSheets, getSalesSheetSnapshotsByDateForStores, getStores } from '@/lib/db-queries'

export default function AdminSalesSheetDayPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const [snapRes, sheetRes] = await Promise.all([
        storeIds.length ? getSalesSheetSnapshotsByDateForStores(date, storeIds) : Promise.resolve({ data: [], error: null }),
        getAllSalesSheets(800),
      ])
        ...
        {loading ? (
          ...
          <div className="space-y-6">
                      ...
                      <table className="w-full">
                        <thead className="bg-gray-50 border-b border-gray-200">
                          <tr>
                            ...
                            <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900">Quantity (pcs)</th>
                        ...
                        <tbody className="divide-y divide-gray-200">
                          {items.map((item: any) => {
                            ...
                            return (
                              <tr key={item.product_id} className="hover:bg-gray-50">
                                ...
                                <td className="px-4 py-3 text-sm text-gray-900">{qty}</td>
                                <td className="px-4 py-3 text-sm text-gray-900">
                                  {Number(item.profit || 0).toFixed(0)} BDT
                                </td>
                                <td className="px-4 py-3 text-sm text-gray-900">{dc.toFixed(0)} BDT</td>
                                <td className="px-4 py-3 text-sm text-gray-900">{per.toFixed(2)}</td>
                                <td className="px-4 py-3 text-sm text-gray-900">{perUnitTk.toFixed(2)} BDT</td>
                              </tr>
                            )
                          })}
                          <tr className="bg-gray-50 font-semibold">
                            ...
                            <td className="px-4 py-3 text-sm text-gray-900">{totalDollarCost.toFixed(0)} BDT</td>
            ...
            <Card className="p-6 bg-yellow-50 border border-yellow-200">
              <h2 className="text-xl font-bold text-gray-900 mb-4">Combined Summary - All Stores ({date})</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                  <p className="text-sm text-gray-600">Total Sell (All Stores)</p>
                  <p className="text-lg font-bold text-gray-900">{totals.totalSales.toFixed(0)} BDT</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Gross Profit (Day)</p>
                  <p className="text-lg font-bold text-gray-900">{grossProfitAllStores.toFixed(0)} BDT</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-gray-900">{totals.totalDollarCost.toFixed(0)} BDT</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Operational / Management / Financial / Content / Misc</p>
                  <p className="text-sm font-medium text-gray-900">
                    Op: {Number(combined.variables?.operational_expense || 0).toFixed(0)} | Mgmt:{' '}
                    {Number(combined.variables?.management_cost || 0).toFixed(0)} | Fin:{' '}
                    {Number(combined.variables?.financial_cost || 0).toFixed(0)} | Content:{' '}
                    {Number(combined.variables?.content_cost || 0).toFixed(0)} | Misc:{' '}
                    {Number(combined.variables?.others || 0).toFixed(0)}
                  </p>
                </div>
